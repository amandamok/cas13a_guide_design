...
#   xlab("") + ylab("rate above no activator\n(RFU/min)")
```
...
ggplot(subset(guide_rate, crRNA_spacer_basepairs %in% c(0, 4, 10, 16)),
  ...
  theme_bw() + xlab("") + ylab("rate above no activator\n(RFU/min)") + 
...
(ggplot(subset(guide_rate, !is.na(time_to_detection)),
   ...
   theme_bw() + xlab("rate above no activator\n(RFU/min)") + 
...
ggplot(guide_rate, aes(x=start, y=Estimate, col=crRNA_spacer_basepairs, 
  ...
  xlab("genomic position") + ylab("rate above no activator\n(RFU/min)")
```
...
ggplot(guide_rate, aes(x=GC_content, y=Estimate)) + 
  ...
  xlab("GC content (%)") + ylab("rate above no activator\n(RFU/min)") +
...
ggplot(guide_rate, aes(x=gRNA_MFE, y=Estimate)) + 
  geom_point() + geom_smooth(method="lm", formula=y~x) + theme_bw() + 
  xlab("gRNA MFE (kcal/mol)") + ylab("rate above no activator\n(RFU/min)") +
  annotate(geom="text", x=-Inf, y=75, hjust="inward", vjust="inward", col="red",
           label=bquote(rho~"="~.(round(with(guide_rate, 
                                             cor.test(gRNA_MFE, 
                                                      Estimate,
...
ggplot(guide_rate_adapt, aes(x=guide.expected.activities, y=rate, col=spacer)) + 
  geom_point() + geom_smooth(method="lm", formula=y~x) + theme_bw() +
  xlab("ADAPT prediction") + ylab("rate above no activator\n(RFU/min)")
```
...
antitag_G$num_complementary <- with(antitag_G,
                                    ifelse(antitag_label=="GUUR", 3,
                                           ifelse(antitag_label=="GURR", 2, 1)))
antitag_G$old_direct_repeat <- "uagaccaccccaaaaaugaaggggacuaaaac"
write.csv(antitag_G, 
          file=file.path(project_dir, "guides_antitag_G.csv"))
```
...
(ggplot(guide_rate, aes(x=start, y=Estimate)) + theme_classic() + 
    geom_point() + geom_errorbar(aes(ymin=Estimate+qnorm(0.05)*Std.Error,
                                     ymax=Estimate-qnorm(0.05)*Std.Error)) + 
    xlab("position along genome") + ylab("rate above no activator\n(RFU/min)") + ylim(-10, 100) + 
...
(ggplot(model1_coefs, aes(x=position, y=value, fill=coef)) + 
  ...
  (ggplot(subset(guide_rate, nchar(spacer)==20),
     ...
     xlab("first position in anti-tag") + ylab("rate above no activator\n(RFU/min)"))
...
Suppl. Figure 1: anti-tag complementarity
...
ggplot(subset(guide_rate, nchar(spacer)==20),
  ...
  xlab("anti-tag") + ylab("rate above no activator\n(RFU/min)")
...
Suppl. Figure 2A: random forest variable importance
...
(ggplot(guide_rate, aes(x=GC_content, y=Estimate)) + theme_classic(base_size=10) + 
     ...
     xlab("spacer %GC") + ylab("rate above no activator\n(RFU/min)"))
...
Suppl. Figure 3B: cleaveable U in target context
...
ggsave(file=file.path(figure_dir, "suppl_figure_3B.pdf"),
       plot=last_plot(),
       device="pdf", width=6, height=2, units="in")
```

Suppl. Figure 4A: spacer structure
...
Suppl. Figure 4B: structure of direct repeat
...
Suppl. Figure 5A: in vivo viral structure
...
((ggplot(guide_rate, aes(x=lan, y=Estimate)) + theme_classic(base_size=10) +
    ...
    (ggplot(guide_rate, aes(x=sun_invivo, y=Estimate)) + theme_classic(base_size=10) +
       geom_point(shape=16) + geom_smooth(method=lm, formula=y~x) +
       xlab("mean icSHAPE score") + ylab("rate above no activator\n(RFU/min)") +
...
Suppl. Figure 6A: multiplex set of 40 vs. primary screen
...
ggplot_violin(guide_rate, x_var="multiplex_40") + 
  theme_classic(base_size=10) + guides(fill=F) + 
  ylab("rate above no activator\n(RFU/min)")
