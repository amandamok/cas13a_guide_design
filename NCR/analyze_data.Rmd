...
guide_features$direct_repeat <- substr(guide_features$structure, 
...
guide_features$downstream_U <- sapply(guide_features$start,
                                      function(x) {
                                        region_start <- x + 20
                                        region_end <- region_start + 30 - 1
                                        tmp_region <- viral_genome[region_start:region_end,]
                                        mean(tmp_region$base == "U")
                                      })
guide_features$downstream_unstructured_u <- sapply(guide_features$start,
                                                   function(x) {
                                                     region_start <- x + 20
                                                     region_end <- region_start + 30 - 1
                                                     tmp_region <- viral_genome[region_start:region_end,]
...
guide_structure <- sapply(seq(nrow(guide_features)),
                          function(x) {
                            ...
                            region_annotations <- c(manfredonia = mean(region_features$manfredonia == "lowShannon/highSHAPE"),
                                                    ...
                                                    invivo = mean(region_features$invivo == 1))
...
(ggplot(guide_rate, aes(x=downstream_U, y=Estimate)) + 
   geom_point() + geom_smooth(method="lm", formula=y~x) + theme_bw() + 
   xlab("% U in downstream 30nt in viral genome") + 
   ylab("rate above no activator (RFU/min)") +
   annotate(geom="text", x=Inf, y=75, hjust="inward", vjust="inward", col="red",
            label=bquote(rho~"="~.(round(with(guide_rate, 
  ...
  (ggplot(guide_rate, aes(x=downstream_unstructured_u, y=Estimate)) + 
     geom_point() + geom_smooth(method="lm", formula=y~x) + theme_bw() + 
     xlab("% unstructured U in downstream 30nt in viral genome") + 
     ylab("rate above no activator (RFU/min)") +
     annotate(geom="text", x=Inf, y=75, hjust="inward", vjust="inward", col="red",
              label=bquote(rho~"="~.(round(with(guide_rate, 
...
* slight non-significant correlation between % unstructured U in 30 nt downstream of protospacer in viral genome and guide activity
...
model3_plot <- ((model3$MSE$mse_plot + 
                   ggtitle("Model 1", subtitle="rate ~ structure (+/- 5nt)")) +
...
model4_plot <- ((model4$MSE$mse_plot + 
                   ggtitle("Model 1", subtitle="rate ~ sequence [A/U v. C/G] (+/- 5nt)")) +
...
model5_plot <- ((model5$MSE$mse_plot + 
                   ggtitle("Model 1", subtitle="rate ~ sequence [A/U v. C/G] + structure(+/- 5nt)")) +
