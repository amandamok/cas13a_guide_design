...
(ggplot(guide_rate, aes(x=start, y=Estimate)) + theme_classic() + 
    geom_point() + geom_errorbar(aes(ymin=Estimate+qnorm(0.05)*Std.Error,
                                     ymax=Estimate-qnorm(0.05)*Std.Error)) + 
    xlab("position along genome") + ylab("rate above no activator (RFU/min)") + ylim(-10, 100) + 
...
(ggplot(virus_v_gblock_data, aes(x=Estimate)) + 
    ...
    theme_classic() + scale_y_continuous(breaks=c(0, 20)) +
  ...
  (ggplot(virus_v_gblock_data, aes(y=gblock_rate_round2)) + 
     geom_histogram(binwidth=5) + xlab("") + ylab("") + 
     theme_classic() + scale_x_continuous(breaks=c(0, 50)) + 
...
guide_rate$antitag_label <- sapply(guide_rate$antitag,
                                   function(x) {
...
guide_rate$antitag_label <- factor(guide_rate$antitag_label,
...
(ggplot(model1_coefs, aes(x=position, y=value, fill=coef)) + 
  ...
  (ggplot(subset(guide_rate, nchar(spacer)==20),
     ...
     xlab("first position in anti-tag") + ylab("rate above no activator (RFU/min)"))
...
(ggplot(gblock_rate, aes(x=start, y=Estimate)) + theme_classic() + 
    geom_point() + geom_errorbar(aes(ymin=Estimate+qnorm(0.05)*Std.Error,
                                     ymax=Estimate-qnorm(0.05)*Std.Error)) + 
    xlab("") + ylab("rate above no activator (RFU/min)") + ylim(-10, 100) + 
    theme(axis.title=element_text(size=10),
          axis.text=element_text(size=10))) + 
  (ggplot(gene_bed, aes(xmin=thickStart, xmax=thickEnd, ymin=ymax-4.5, ymax=ymax)) + 
     theme_void() + geom_rect() + 
...
ggplot(subset(guide_rate, nchar(spacer)==20),
  ...
  theme(axis.text=element_text(size=10), 
        axis.title=element_text(size=10)) + 
  xlab("anti-tag") + ylab("rate above no activator (RFU/min)")

ggsave(file=file.path(figure_dir, "suppl_figure_2D.pdf"),
       plot=last_plot(),
       device="pdf", width=4, height=2, units="in")
```
...
ggsave(file=file.path(figure_dir, "suppl_figure_4.pdf"),
       plot=last_plot(),
       device="pdf", width=6.5, height=3, units="in")
```
...
ggsave(file=file.path(figure_dir, "suppl_figure_5.pdf"),
       plot=last_plot(),
       device="pdf", width=6.5, height=3, units="in")
```
