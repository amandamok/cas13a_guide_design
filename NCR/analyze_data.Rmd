...
ggplot_violin <- function(plot_data, x_var, y_var="Estimate", fill_var=NULL,
                          x_lab="", y_lab="activator-dependent activity (RFU/min)",
                          binwidth=1) {
...
plot_coefs <- function(fit_obj, best_alpha, best_lambda) {
  ...
  ggplot(coefs) + theme_bw() +
    geom_point(data=subset(coefs, type=="seq"), 
               aes(x=position, y=value, col=coef), alpha=0.5) +
    geom_point(data=subset(coefs, type=="str"),
               aes(x=position, y=value, shape=coef), alpha=0.75) +
    annotate("rect", xmin=0.5, xmax=20.5, fill="grey", alpha=0.2, 
             ymin=min(coefs$value) - 1, ymax=max(coefs$value) + 1) + 
    annotate("text", x=0.5, y=max(coefs$value) + 1, label="spacer,  5' : 3'", 
  ...
  fold_plots <- lapply(seq_along(fold_plots),
                       function(x) {
                         if(x != length(fold_plots)) {
                           return(fold_plots[[x]] + guides(col=F))
                         } else {
                           return(fold_plots[[x]])
                         }
                       })
...
all_guides <- read.table(file.path(here(), "outputs", "cas13a_20nt",
...
+ Want to be able to account for additional gain in activator-dependent activity background
...
* Mixed linear regression to compute rate per condition across triplicates:
    ...
    + 100fM activator-dependent activity: $\beta_{t:\text{100fM}}$
...
ggplot(guide_rate, aes(x=Estimate, y=guide_id, col=plate)) + geom_point() +
  ...
  xlab("activator-dependent activity\n(RFU/min)") + ylab("guide id")
...
#   xlab("") + ylab("activator-dependent activity\n(RFU/min)")
```

```{r exclude_1344_outlier}
...
ggplot(subset(guide_rate, crRNA_spacer_basepairs %in% c(0, 4, 10, 16)),
  ...
  theme_bw() + xlab("") + ylab("activator-dependent activity\n(RFU/min)") + 
...
(ggplot(subset(guide_rate, !is.na(time_to_detection)),
   ...
   theme_bw() + xlab("activator-dependent activity\n(RFU/min)") + 
...
ggplot(guide_rate, aes(x=start, y=Estimate, col=crRNA_spacer_basepairs, 
  ...
  xlab("genomic position") + ylab("activator-dependent activity\n(RFU/min)")
```
...
ggplot(guide_rate, aes(x=spacer_bp, y=Estimate)) + geom_point() + 
  ...
  ylab("activator-dependent activity\n(RFU/min)") +
...
ggplot(guide_rate, aes(x=GC_content, y=Estimate)) + 
  ...
  xlab("GC content (%)") + ylab("activator-dependent activity\n(RFU/min)") +
...
ggplot(guide_rate, aes(x=gRNA_MFE, y=Estimate)) + 
  geom_point() + geom_smooth(method="lm", formula=y~x) + theme_bw() + 
  xlab("gRNA MFE (kcal/mol)") + ylab("activator-dependent activity\n(RFU/min)") +
  annotate(geom="text", x=-Inf, y=75, hjust="inward", vjust="inward", col="red",
           label=bquote(rho~"="~.(round(with(guide_rate, 
                                             cor.test(gRNA_MFE, 
                                                      Estimate,
...
ggplot(guide_rate_adapt, aes(x=guide.expected.activities, y=rate, col=spacer)) + 
  geom_point() + geom_smooth(method="lm", formula=y~x) + theme_bw() +
  xlab("ADAPT prediction") + ylab("activator-dependent activity\n(RFU/min)")
```
...
```{r model_2, warning=F, fig.height=8}
...
```{r model_3, warning=F, fig.height=8}
...
model1_coef_plot / 
  ((pos.3_plot + guides(fill=F)) | 
     (pos0_plot + guides(fill=F)) | 
     (pos3_plot + labs(fill="base"))) / 
  ((pos8_plot + guides(fill=F)) | 
     (pos22_plot + guides(fill=F)) | 
...
model6_fold_plot <- wrap_plots(lapply(model6_fits, 
                                      function(x) x$plot + guides(col=F)), nrow=1)
...
(ggplot(subset(guide_rate, !is.na(gblock_rate_round1)), 
    ...
    xlab("viral RNA: activator-dependent activity (RFU/min)") + 
    ylab("gBlocks: activator-dependent activity (RFU/min)") +
...
design_data <- read.table(file.path(here(), "outputs", "cas13a_20nt", 
...
(ggplot(guide_rate, aes(x=start, y=Estimate)) + theme_classic() + 
    geom_point() + geom_errorbar(aes(ymin=Estimate+qnorm(0.05)*Std.Error,
                                     ymax=Estimate-qnorm(0.05)*Std.Error)) + 
    xlab("position along genome") + ylab("activator-dependent activity\n(RFU/min)") + ylim(-10, 100) + 
...
plot_text_527 <- data.frame(x=10,
                            ...
                            text=c(paste("no activator rate:", signif(rate_noActivator_527, 3)),
...
plot_text_505 <- data.frame(x=10,
                            ...
                            text=c(paste("no activator rate:", signif(rate_noActivator_505, 3)),
...
guide_plot_527 <- ggplot(data_predict_527, 
  ...
  xlab("time (min)") + ylab("RFU") + guides(col=F)
...
guide_plot_505 <- ggplot(data_predict_505, 
  ...
  scale_color_manual(values=c("red", "black"), 
                     labels=c("100fM activator", "no activator")) +
  theme(axis.text=element_text(size=10), 
        axis.title=element_text(size=10), 
        plot.title=element_text(size=10), 
...
(ggplot(model1_coefs, aes(x=position, y=value, fill=coef)) + 
    ...
    theme(axis.text=element_text(size=10), 
          ...
          plot.title=element_text(size=10),
          legend.position="bottom") +
    annotate("rect", xmin=1, xmax=20, ymin=2, ymax=3, fill="black") + 
    annotate("text", x=10.5, y=2.5, label="protospacer", col="white", size=2.5) +
    annotate("rect", xmin=-5, xmax=1, ymin=2, ymax=3, fill="darkgrey") +
  ...
  (ggplot(guide_rate,
     ...
     xlab("first position in anti-tag") + ylab("activator-dependent activity\n(RFU/min)"))
...
# calculate slopes
variant_slopes <- lapply(unique(variants_platemap$sample),
                         function(x) {
                           tmp_variant <- strsplit(x, split="_")[[1]][1]
                           tmp_multiplex <- paste(strsplit(x, split="_")[[1]][2:3], sep="_")
                           tmp_conc <- strsplit(x, split="_")[[1]][4]
                           tmp_wells <- subset(variants_platemap, sample==x)$well
                           tmp_data <- subset(variants_data, 
                                              well %in% tmp_wells & time >= 10)
...
variant_slopes$conc[variant_slopes$variant=="No Activator"] <- "No Activator"
variant_slopes$variant[variant_slopes$variant=="No Activator"] <- "Controls"
variant_slopes$conc[variant_slopes$variant=="No Protein"] <- "No Protein"
variant_slopes$variant[variant_slopes$variant=="No Protein"] <- "Controls"
variant_slopes$conc <- factor(variant_slopes$conc,
                              levels=c("1 aM", "10 aM", "100 aM", "1 fM", "10 fM",
                                       "No Activator", "No Protein"))
variant_slopes$variant <- factor(variant_slopes$variant,
                                 levels=c("WT", "Controls", 
                                          "alpha: B.1.1.7",
                                          "beta: B.1.351",
                                          "gamma: P.1",
                                          "delta: B.1.617.1"))
variant_slopes$multiplex <- factor(variant_slopes$multiplex,
                                   levels=c("8", "32"))
levels(variant_slopes$multiplex) <- c("8 guides", "32 guides")

ggplot(subset(variant_slopes, conc=="10 fM"), aes(x=multiplex, y=Estimate, fill=multiplex)) + 
  geom_col(position="dodge") + facet_grid(~variant, scales="free_x", space="free_x") +
  theme_classic(base_size=10) + xlab("") + ylab("activator-dependent activity\n(RFU/min)") + 
  scale_fill_manual(values=c("grey", "black")) + 
  theme(axis.text.x=element_text(angle=90, vjust=0.5, hjust=1),
        legend.position="none", strip.text.x=element_text(angle=90)) 
...
noActivator_spacer_violin <- ggplot(subset(noActivator_rate, 
  ...
  ylab("no activator rate (RFU/min)")
...
guide_spacer_antitag_violin <- ggplot(subset(guide_rate, 
  ...
  geom_smooth(method=lm, formula=y~x) + 
  theme_classic() + guides(fill=F) + facet_grid(~antitag_pos1) + 
  theme(axis.text=element_text(size=10), axis.title=element_text(size=10)) +
  scale_x_continuous(breaks=c(0, 4, 10, 16)) + 
  xlab("# basepaired positions in spacer") + 
  ylab("activator-dependent activity\n(RFU/min)") + 
...
(ggplot(subset(noActivator_rate, NCR.id %in% guide_rate$NCR.id), 
   ...
   xlab("") + ylab("activator-independent activity\n(RFU/min)")) / 
...
Suppl. Figure 4B: genomic structure vs. activity

```{r suppl_figure_4B}
((ggplot(guide_rate, aes(x=lan, y=Estimate)) + theme_classic(base_size=10) +
    geom_point(shape=16) + geom_smooth(method=lm, formula=y~x) +
    xlab("% unstructured") + ylab("activator-dependent activity\n(RFU/min)") + 
...
levels(loo_plot_data$sample)[levels(loo_plot_data$sample)=="No Act"] <- "No Activator"
...
(ggplot(loo_plot_data, aes(x=sample, y=time.trend, fill=pool)) + geom_col() + 
  ...
  geom_text(aes(y=time.trend+0.5, label=text), col="red") + 
...
human_rna_slopes$guide[human_rna_slopes$guide=="Apo Protein"] <- "No Activator"
human_rna_slopes$guide[human_rna_slopes$guide=="612"] <- "crRNA control"

ggplot(human_rna_slopes, aes(x=guide, y=Estimate, fill=type)) + geom_col() + 
  theme_classic(base_size=10) + scale_fill_manual(values=c("grey", "black")) + 
  theme(axis.text.x=element_text(angle=90, vjust=0.5, hjust=1), 
        legend.position="none") + 
  geom_errorbar(aes(ymin=Estimate+qnorm(0.025)*Std..Error, 
                    ymax=Estimate-qnorm(0.025)*Std..Error)) + 
  geom_text(aes(y=Estimate + 0.01, label=text), col="red") + 
  xlab("") + ylab("RFU/min\n(+ human RNA) - (- human RNA)")
...
loo_largepool_conc <- c("No Protein", "No Activator", "30 fM", "10 fM", "3 fM", "1 fM", "300 aM")
                                             ...
                                             lapply(loo_largepool_conc[-c(1:2)],
                                                    function(tmp_conc) {
                                                      tmp_data <- subset(loo_largepool_data,
                                                                         multiplex==tmp_pool &
                                                                           conc %in% c("No Activator", tmp_conc))
...
loo_largepool_detectable$label <- ifelse(loo_largepool_detectable$Pr...t.. < 0.05,
                                         "*", "")
## all multiplex pools are detectable above no activator at all concentrations
...
ggplot(subset(loo_largepool_rates, mismatch != "No Protein"), 
       aes(x=conc, y=Estimate)) + 
  geom_col(aes(fill=mismatch), position="dodge") + theme_classic(base_size=10) + 
  geom_errorbar(aes(ymin = Estimate + qnorm(0.025)*Std..Error,
                    ymax = Estimate - qnorm(0.025)*Std..Error,
                    fill=mismatch),
                position="dodge") + 
  facet_grid(~multiplex_label, scales="free_x", space="free_x") + 
  theme(axis.text.x=element_text(angle=90, hjust=1, vjust=0.5)) + 
  xlab("") + ylab("RFU/min") + labs(fill="") + scale_y_log10() +
  scale_fill_manual(values=c("grey", "black")) + 
  geom_text(data=loo_largepool_mismatch, col="red",
            aes(x=conc, y=1.2*text_height, label=text))
...
ggplot(guide_rate,
  ...
  theme_classic(base_size=10) + guides(fill=F) +
  scale_fill_manual(values=RColorBrewer::brewer.pal(6, "Set1")) +
  xlab("anti-tag") + ylab("activator-dependent activity\n(RFU/min)")
...
antitag_G_structure <- rbind(subset(guide_rate, 
                                    antitag_pos1=="G" & crRNA_spacer_basepairs==0 &
                                      nchar(spacer==20))[1,],
                             antitag_G_structure)
antitag_G_structure$structure <- factor(antitag_G_structure$structure, 
                                        levels=antitag_G_structure$structure)
...
(ggplot(subset(guide_rate, 
  ...
  (ggplot(antitag_G_structure, 
          aes(x=spacer_structure, y=Estimate, fill=crRNA_spacer_basepairs)) + 
     geom_col() + theme_classic() + theme(axis.text.x=element_text(angle=90)) + 
     xlab("") + ylab("activator-dependent activity\n(RFU/min)"))
```
