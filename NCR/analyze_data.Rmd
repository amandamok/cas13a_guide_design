...
ttest_lasttimepoint <- lapply(guide_ids,
                              function(x) {
...
Per guide:
* Per timepoint: t-test for difference in signal between no activator and 100 fM conditions per timepoint
...
```{r time_to_detection}
guide_rate$time_to_detection <- sapply(guide_ids,
                                       function(x)  {
...
(ggplot(subset(guide_rate, !is.na(time_to_detection)),
   ...
   theme_bw() + xlab("rate above no activator (RFU/min)") + 
...
ggplot(guide_rate, aes(x=Estimate, y=-log10(p.value), 
  ...
  geom_point() + theme_bw() + 
  xlab("rate above no activator (RFU/min)") + ylab("-log10(p)") +
...
lan_model <- spacer_viral_structure$lan$full_model$model
lan_model$predict <- predict(spacer_viral_structure$lan$full_model)
ggplot(lan_model, aes(x=rate, y=predict)) + geom_point() + theme_bw() + 
  geom_abline(aes(slope=1, intercept=0, col="y=x")) + labs(color="") + 
  xlab("True rate (RFU/min)") + ylab("Predicted rate (RFU/min)") + 
  ggtitle("Rate ~ spacer sequence + viral genome structure",
          subtitle=bquote("full model" ~ r^2 == 
                            .(round(summary(spacer_viral_structure$lan$full_model)$r.squared, 
                                    digits=3)) ~ 
                            "| structure: Lan"))
...
viral_structure <- sapply(guide_rate$start,
                          function(x) 
                            ifelse(viral_genome$lan[x:(x+19)]=="unstructured",
                                   "unstructured", "."))
regression_data <- data.frame(guide_rate$Estimate,
                              t(spacer_seq),
                              t(viral_structure),
...
lan_regression_data <- regression_data
lan_regression_data <- lan_regression_data[sample(nrow(lan_regression_data)),]
folds <- cut(seq(nrow(lan_regression_data)), breaks=k, labels=F)

crossvalidation_r2 <- sapply(seq(k),
                             function(x) {
                               training_fit <- lm(rate ~ ., 
                                             data=subset(lan_regression_data, folds != x))
                               test <- subset(lan_regression_data, folds == x)$rate
                               test_predict <- predict(training_fit, 
                                                           newdata = subset(lan_regression_data,
                                                                            folds == x))
                               test_r2 <- cor(test, test_predict)^2
                               return(c(training=summary(training_fit)$r.squared,
                                        test=test_r2))
                             })
crossvalidation_r2 <- data.frame(t(crossvalidation_r2))
ggplot(crossvalidation_r2, aes(x=training, y=test)) + geom_point() + theme_bw() + 
  xlab(expr(paste("training ", r^2))) + ylab(expr(paste("test ", r^2))) + 
  ggtitle("Rate ~ spacer sequence + viral genome structure (Lan 2020)",
          subtitle="10-fold cross-validation")

lan_lasso <- glmnet::cv.glmnet(x=model.matrix(rate ~., data=lan_regression_data), 
                               y=lan_regression_data$rate,
                               family="gaussian", foldid=folds, alpha=1)
lan_lasso_metrics <- data.frame(lambda=lan_lasso$lambda, 
                                error=lan_lasso$cvm, 
                                cv_upper=lan_lasso$cvup, 
                                cv_lower=lan_lasso$cvlo,
                                num_coef=lan_lasso$nzero, 
                                dev_ratio=lan_lasso$glmnet.fit$dev.ratio)
(ggplot(lan_lasso_metrics, aes(x=lambda, y=error, col=num_coef)) + theme_bw() + 
  geom_point() + geom_line() + geom_errorbar(aes(ymin=cv_lower, ymax=cv_upper)) + 
  labs(col="# non-zero coefficients") + 
  xlab("LASSO penalty") + ylab("cross-validated MSE") + 
  ggtitle("Rate ~ spacer sequence + viral genome structure (Lan 2020)",
          subtitle="Cross-validated LASSO regresion (k=10)")) + 
  (ggplot(lan_lasso_metrics, aes(x=lambda, y=dev_ratio, col=num_coef)) + theme_bw() +
     geom_point() + geom_line() + labs(col="# non-zero coefficients") +
     xlab("LASSO penalty") + ylab("cross-validated MSE"))
```
...
```{r position_regression_elasticnet, eval=F}
...
```{r position_regression_logistic, fig.height=10}
structure_features <- c("spacer_only", "manfredonia", "lan", "sun_invivo", "sun_invitro", "huston")
logistic_fit_all <- lapply(structure_features,
                           function(feature) {
                             # wrangle data.frame from regression
                             spacer_seq <- sapply(guide_rate$start,
                                                  function(x) viral_genome$spacer_base[x:(x+19)])
                             if(feature == "spacer_only") {
                               regression_data <- data.frame(guide_rate$detectable,
                                                             t(spacer_seq),
                                                             stringsAsFactors=T)
                               colnames(regression_data) <- c("detectable", paste0("spacer_", 1:20))
                             } else {
                               viral_structure <- sapply(guide_rate$start,
                                                         function(x) {
                                                           viral_structure <- viral_genome[[feature]][x:(x+19)]
                                                           if(feature == "lan") {
                                                             viral_structure[viral_structure=="both"] <- "."
                                                             viral_structure[viral_structure=="structured"] <- "."
                                                           }
                                                           if(grepl("sun", feature)) {
                                                             viral_structure <- as.numeric(viral_structure)
                                                           }
                               ...
                               regression_data <- data.frame(guide_rate$detectable,
                                                             ...
                                                             t(viral_structure),
                                                             stringsAsFactors=T)
                               colnames(regression_data) <- c("detectable",
                                                              paste0("spacer_", 1:20),
                                                              paste0("structure_", 1:20))
                             }
                             position_features <- colnames(regression_data)[-1]
                             regression_data <- subset(regression_data,
                                                       nchar(guide_rate$spacer)==20)
                             # compute full model
                             full_model <- glm(detectable ~ ., data=regression_data, family="binomial")
                             # compute positional models
                             position_models <- lapply(position_features,
                                                       function(x) {
                                                         tmp_data <- subset(regression_data,
                                                                            select=colnames(regression_data) %in% 
                                                                              c("detectable", x))
                                                         glm(detectable ~ ., data=tmp_data, family="binomial")
                                                       })
                             position_models_r2 <- data.frame(position_features,
                                                              sapply(position_models,
                                                                     function(x) {
                             ...
                             feature_plot <- ggplot(position_models_r2, 
                                                    aes(x=pos, y=r2, fill=type)) + 
                               geom_col(position="stack") + theme_bw() + ylim(0, 0.095) + 
                               xlab("position in spacer") + ylab(expr(r^2)) + 
                               ggtitle("Detectable @ 2h ~ spacer sequence + viral genome structure",
                                       subtitle=bquote("full model" ~ r^2 == 
                                                         .(round(with(summary(full_model),
                                                                      1-deviance/null.deviance), 
                                                                 digits=3)) ~ 
                                                         "| structure:"~.(feature)))
                             return(list(data=regression_data,
...
names(logistic_fit_all) <- structure_features
logistic_fit_all$spacer_only$plot <- logistic_fit_all$spacer_only$plot + 
  ggtitle("Detectable @ 2h ~ spacer sequence", 
          subtitle=bquote("full model" ~ r^2 == 
                            .(round(with(summary(logistic_fit_all$spacer_only$full_model),
                                         1-deviance/null.deviance),
                                   digits=3))))
wrap_plots(lapply(logistic_fit_all, function(x) x$plot), ncol=1)
logistic_fit_all$spacer_only$plot / logistic_fit_all$lan$plot / logistic_fit_all$sun_invivo$plot
```
...
* Sequence: A/T vs. C/G (binary), with respect to spacer coordinates
* Target viral structure: structured vs. unstructured (binary) from *in vivo* annotations
* Spacer viral structure: number of base-paired positions in spacer

```{r viral_structure_invivo}
invivo_features <- data.frame(lan=factor(ifelse(viral_genome$lan=="unstructured",
                                                "unstructured", "."),
                                         levels=c(".", "unstructured")),
                              sun=viral_genome$sun_invivo,
                              huston=factor(ifelse(viral_genome$huston==0,
                                                   "unpaired", "paired"),
                                            levels=c("paired", "unpaired")))
write.table(invivo_features,
            file=file.path(project_dir, "invivo_features.txt"),
            quote=F, row.names=F, col.names=T)
```

```{r guide_model, warning=F, fig.height=8}
window_start <- -5
window_stop <- 5
alphas <- seq(0, 1, by=0.1)
...
set.seed(21)
num_folds <- 5
full_model_folds <- sample(cut(seq(nrow(regression_data)), breaks=num_folds, labels=F))
full_model_alphas <- seq(0, 1, by=0.1)
full_model_elasticnet <- lapply(alphas,
                                function(alpha) {
                                  cv.glmnet(regression_data_onehot[, -1], # as.matrix(regression_data[, -1])
                                            regression_data$rate,
                                            foldid=full_model_folds,
                                            alpha=alpha)
                                })
full_model_elasticnet_r2 <- lapply(seq_along(alphas),
                                   function(alpha_id) {
                                     tmp_lambdas <- full_model_elasticnet[[alpha_id]]$lambda
                                     do.call(rbind,
                                             lapply(seq(num_folds),
                                                    function(fold_id) {
                                                      tmp_training <- subset(regression_data_onehot,
                                                                             full_model_folds!=fold_id)
                                                      tmp_testing <- subset(regression_data_onehot, 
                                                                            full_model_folds==fold_id)
                                                      tmp_fit <- glmnet(as.matrix(tmp_training[, -1]),
                                                                        tmp_training[, 1],
                                                                        alpha=alphas[alpha_id], lambda=tmp_lambdas)
                                                      tmp_predict <- lapply(tmp_lambdas,
                                                                            function(lambda) {
                                                                              predict(tmp_fit,
                                                                                      newx=as.matrix(tmp_testing[, -1]),
                                                                                      s=lambda)
                                                                            })
                                                      tmp_r2 <- sapply(tmp_predict,
                                                                       function(x) {
                                                                         1 - sum((tmp_testing[, 1]-x)^2) / 
                                                                           sum((tmp_testing[, 1]-mean(tmp_testing[, 1]))^2)
                                                                       })
                                                      return(data.frame(alpha=alphas[alpha_id],
                                                                        fold_id=fold_id,
                                                                        lambda=rep(tmp_lambdas, times=2),
                                                                        r2=c(tmp_fit$dev.ratio, tmp_r2),
                                                                        split=rep(c("training", "testing"),
                                                                                  each=length(tmp_lambdas))))
                                                    }))
                                             })
full_model_elasticnet_r2 <- do.call(rbind, full_model_elasticnet_r2)
full_model_elasticnet_r2 <- aggregate(r2 ~ alpha + lambda + split, data=full_model_elasticnet_r2, FUN=mean)
full_model_elasticnet_r2$alpha <- factor(as.character(full_model_elasticnet_r2$alpha),
                                         levels=as.character(alphas))
full_model_elasticnet_r2$split <- factor(full_model_elasticnet_r2$split,
                                         levels=c("training", "testing"))
cv_fullmodel_r2_plot <- ggplot(full_model_elasticnet_r2, aes(x=lambda, y=r2, col=alpha)) + 
  geom_line() + theme_bw() + scale_x_log10() + 
  facet_grid(split ~ ., scales="free_y") + 
  xlab(expr(lambda)) + ylab(bquote("mean" ~ r^2)) + labs(col=expr(alpha)) + 
  ggtitle("Elastic net regression, 5-fold cross-validation", 
          subtitle="rate ~ sequence [ spacer +/- 5nt ]")
full_model_elasticnet_MSE <- lapply(seq_along(full_model_elasticnet),
                                    function(x) {
                                      tmp_fit <- full_model_elasticnet[[x]]
                                      data.frame(lambda=tmp_fit$lambda,
                                                 cv_error=tmp_fit$cvm,
                                                 cv_upper=tmp_fit$cvup,
                                                 cv_lower=tmp_fit$cvlo,
                                                 num_nonzero=tmp_fit$nzero,
                                                 alpha=full_model_alphas[x],
                                                 dev_ratio=tmp_fit$glmnet.fit$dev.ratio)
                                    })
full_model_elasticnet_MSE <- do.call(rbind, full_model_elasticnet_MSE)
full_model_elasticnet_MSE$alpha <- as.factor(full_model_elasticnet_MSE$alpha)
full_model_elasticnet_bestfits <- lapply(alphas,
                                         function(x) {
                                           tmp_fit <- subset(full_model_elasticnet_MSE,
                                                             alpha==x)
                                           tmp_fit[which.min(tmp_fit$cv_error),]
                                         })
full_model_elasticnet_bestfits <- do.call(rbind, full_model_elasticnet_bestfits)
full_model_elasticnet_bestfits$alpha <- as.numeric(as.character(full_model_elasticnet_bestfits$alpha))
best_alpha <- as.numeric(as.character(full_model_elasticnet_MSE$alpha[which.min(full_model_elasticnet_MSE$cv_error)]))
best_lambda <- full_model_elasticnet_MSE$lambda[which.min(full_model_elasticnet_MSE$cv_error)]
cv_mse_plot <- ggplot(full_model_elasticnet_MSE, aes(x=lambda, y=cv_error, col=alpha)) + 
  geom_line() + theme_bw() + scale_x_log10() + 
  xlab(bquote(lambda)) + labs(col=bquote(alpha)) + 
  ylab(paste0("cross-validated error (k=", num_folds, ")")) + 
  ggtitle("rate ~ nucleotide sequence + viral structure",
          subtitle=bquote("best fit:"~alpha~"="~.(best_alpha)~";"~lambda~"="~.(best_lambda)))
          # subtitle="base is A/T vs. C/G; sequence neighborhood: spacer +/- 5 nt") 
cv_r2_plot <- ggplot(full_model_elasticnet_MSE, aes(x=lambda, y=dev_ratio, col=alpha)) +
  geom_line() + theme_bw() + scale_x_log10() +
  xlab(bquote(lambda)) + ylab(expr(r^2)) + labs(col=bquote(alpha))
cv_bestfit_plot <- ggplot(full_model_elasticnet_bestfits, aes(x=alpha, y=dev_ratio, col=num_nonzero)) +
     geom_point() + geom_path() + theme_bw() +
     ggtitle("best fit:", subtitle=bquote(alpha~"="~.(best_alpha)~";"~lambda~"="~.(best_lambda))) +
     ylab(paste0("cross-validated error (k=", num_folds, ")")) +
     labs(col="number non-zero \ncoefficients")

# pull coefficients for best fit
full_model_bestfit <- full_model_elasticnet[[which(alphas==best_alpha)]]
full_model_bestfit_coefs <- coef(full_model_bestfit$glmnet.fit, s=best_lambda)[,1]
full_model_bestfit_coefs <- data.frame(matrix(unlist(strsplit(names(full_model_bestfit_coefs)[-1], 
                                                              split="_")), ncol=3, byrow=T),
                                       full_model_bestfit_coefs[-1],
                                       row.names=NULL)
colnames(full_model_bestfit_coefs) <- c("type", "position", "coef", "value")
full_model_bestfit_coefs$position <- as.numeric(sub("\\.", "-", full_model_bestfit_coefs$position))
# position_sequence_effects <- ggplot(full_model_bestfit_coefs, 
#                                     aes(x=position, y=value, col=(value != 0))) + 
#   annotate("rect", xmin=0.5, xmax=20.5, ymin=-5, ymax=7, fill="grey", alpha=0.5) + 
#   geom_point() + theme_bw() + xlab("position relative to spacer") + ylab(expr(beta)) + 
#   annotate("text", x=0.5, y=7, label="spacer,  5' : 3'", col="red", 
#            hjust="inward", vjust="inward") + 
#   ggtitle("", subtitle="effect of C/G base versus A/T") + labs(col="nonzero \ncoefficient")
position_sequence_effects <- ggplot(full_model_bestfit_coefs) + theme_bw() +
  geom_point(data=subset(full_model_bestfit_coefs, type=="seq"), 
              aes(x=position, y=value, col=coef), alpha=0.5) +
  geom_point(data=subset(full_model_bestfit_coefs, type=="str"),
              aes(x=position, y=value, shape=coef), alpha=0.75) +
  annotate("rect", xmin=0.5, xmax=20.5, ymin=-15, ymax=2, fill="grey", alpha=0.2) + 
  annotate("text", x=0.5, y=2, label="spacer,  5' : 3'", col="red", 
           hjust="inward", vjust="inward") + 
  xlab("position") + ylab(expr(beta))

full_model_bestfit_cv <- lapply(1:num_folds,
                                function(x) {
                                  # tmp_data <- split(regression_data,
                                  #                   full_model_folds == x)
                                  tmp_data <- split(seq(nrow(regression_data_onehot)),
                                                    full_model_folds == x)
                                  tmp_data <- lapply(tmp_data,
                                                     function(indices) {
                                                       data.frame(regression_data_onehot[indices,])
                                                     })
                                  ...
                                  tmp_data$testing$predicted <- predict(tmp_fit,
                                                                        ...
                                                                        s=best_lambda)
                                  tmp_plot_data <- rbind(data.frame(tmp_data$training,
                                                                    split="training"),
                                                         data.frame(tmp_data$testing,
                                                                    split="testing"))
                                  ...
                                  # tmp_r2 <- sapply(tmp_data,
                                  #                  function(x) {
                                  #                    with(x, cor(rate, predicted)^2)
                                  #                  })
                                  tmp_fit_plot <- ggplot(tmp_plot_data, aes(x=rate, y=predicted, col=split)) + 
                                    geom_point() + geom_abline(slope=1, intercept=0) + theme_bw() + 
                                    ggtitle(paste("fold:", x), 
                                            subtitle=paste("# non-zero coefficients: ", tmp_fit$df)) + 
                                    xlab("true rate (RFU/min)") + ylab("predicted rate (RFU/min)") + 
                                    scale_color_manual(values=c("#E69F00", "#56B4E9")) + guides(col=F) + 
                                    annotate(geom="text", x=Inf, y=-Inf, col="#56B4E9",
                                             label=bquote(r^2~"="~.(round(tmp_r2["training"], digits=3))),
                                             hjust="inward", vjust=-2) + 
                                    annotate(geom="text", x=Inf, y=-Inf, col="#E69F00",
                                  ...
                                  return(list(data=tmp_plot_data,
                                              fit=tmp_fit,
                                              plot=tmp_fit_plot,
                                              r2=tmp_r2))
                                })
full_model_bestfit_cv_plots <- lapply(full_model_bestfit_cv, function(x) x$plot)
full_model_bestfit_cv_plots <- wrap_plots(full_model_bestfit_cv_plots, nrow=1)

((cv_mse_plot + ggtitle("rate ~ nucleotide sequence + viral structure",
                        subtitle="one-hot encoding")) +
    (cv_fullmodel_r2_plot + geom_point(data=subset(full_model_elasticnet_r2,
                                                   alpha==best_alpha & lambda==best_lambda),
                                       col=1, size=0.75)) + 
    (position_sequence_effects + 
       ggtitle(bquote("best fit:"~alpha~"="~.(best_alpha)~";"~lambda~"="~.(best_lambda)),
               subtitle=""))) / 
  full_model_bestfit_cv_plots +
...
(ggseqlogo(topSpacers_info, method="custom") + 
  ggtitle("Sequence logo of top 20 spacers") + ylim(0, 0.5)) / 
  (ggseqlogo(bottomSpacers_info, method="custom") +
     ggtitle("Sequence logo for bottom 20 spacers") + ylim(0, 0.5))
