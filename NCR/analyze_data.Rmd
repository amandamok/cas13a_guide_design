...
model8_plot <- ((model8$MSE$mse_plot + 
                   ggtitle("Model 8", subtitle="rate ~ sequence (+/- 5nt) + cis cleavage neighborhood")) +
...
# load data
variants_data <- read.csv(file.path(project_dir, "supplementary_data",
                                    "variant_detection_EM_28092021_results.csv"),
                          header=T, skip=83, nrows=60)
...
# plot
ggplot(variant_slopes, aes(x=variant, y=Estimate, 
  ...
  theme_classic() + xlab("") + ylab("RFU/min") + 
  scale_fill_manual(values=c(ucb_colors$`Rose Garden`, ucb_colors$`Lawrence`),
                    labels=c("8", "32")) + 
  labs(fill="multiplex\npool")
  # add_pvalue(variant_compare_slopes, label="diff", xmin="xmin", xmax="xmax", tip.length=0.01)
...
ggplot(model1_rf_regression_importance, 
  ...
  scale_x_continuous(breaks=c(1, 10, 20)) + 
  scale_fill_manual(values=c(RColorBrewer::brewer.pal(4, "Set1"), "grey")) +
  annotate("rect", xmin=0.5, xmax=20, ymin=-20, ymax=-10, fill="black") + 
  annotate("text", x=10.25, y=-15, label="protospacer", col="white", size=2.5) +
  annotate("rect", xmin=-5, xmax=0.5, ymin=-20, ymax=-10, fill="darkgrey") +
  annotate("text", x=-2.25, y=-15, label="anti-tag", col="white", size=2.5) + 
    ...
    annotate(geom="text", x=-1, y=22.5, size=2.5,
             label=paste("p =", 
                         signif(noActivator_structure_rate$Pr...t..[2], 
                                digits=3))) + 
    xlab("# basepaired positions in spacer") + 
    ylab("activator-independent rate\n(RFU/min)")) / 
...
# plot subset for figure 5A
loo_subset <- subset(loo_plot_data, pool=="1354+1367+1368+1370")
loo_subset$new_label <- c("1+2+3+4", "-1", "-2", "-3", "-4")
loo_subset$new_label <- factor(loo_subset$new_label, 
                               levels=loo_subset$new_label)
loo_subset$new_color <- sapply(loo_subset$p_contrast,
                               function(x) {
                                 ifelse(is.na(x), "4-plex", 
                                        ifelse(x<0.05, "keep", "filter out"))
                               })
ggplot(loo_subset, aes(x=new_label, y=time.trend, fill=new_color,
                       ymin=lower.CL, ymax=upper.CL)) + 
  geom_col() + geom_errorbar() + geom_hline(yintercept=0) + 
  theme_classic(base_size=7) + xlab("") + ylab("RFU/min") + 
  theme(axis.text.x=element_text(angle=45, vjust=0.5, hjust=1)) + 
  scale_fill_manual(values=c("grey", "black", "red")) + 
  labs(fill="")
ggsave(file=file.path(figure_dir, "figure_5A_loo.pdf"),
       plot=last_plot(),
       device="pdf", width=3, height=1.5, units="in")
```
...
human_rna_subset <- subset(human_rna_slopes, guide %in% c(516, 546, 558, 576, 590))
human_rna_subset$new_label <- seq(nrow(human_rna_subset))
human_rna_subset$keep <- ifelse(human_rna_subset$Pr...t.. < 0.05, "filter", "keep")
ggplot(human_rna_subset, aes(guide, y=Estimate, fill=keep, 
                             ymin=Estimate+qnorm(0.025)*Std..Error,
                             ymax=Estimate+qnorm(0.975)*Std..Error)) + 
  geom_col() + geom_errorbar() + geom_hline(yintercept=0) + 
  theme_classic(base_size=7) + scale_fill_manual(values=c("black", "red")) + 
  theme(legend.position="none") + xlab("") + ylab("human RNA-\ndependent\nrate\n(RFU/min)")
ggsave(file=file.path(figure_dir, "figure_5A_humanRNA.pdf"),
       plot=last_plot(),
       device="pdf", width=2.5, height=1, units="in")
```
