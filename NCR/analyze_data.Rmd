...
spacer_viral_structure <- lapply(structure_features,
                                 function(feature) {
                                   ...
                                   spacer_seq <- sapply(guide_rate$start,
                                                        function(x) viral_genome$base[x:(x+19)])
...
# loo_models <- lapply(2:ncol(regression_data),
#                      function(x) {
#                        tmp_data <- regression_data[, -x]
#                        lm(rate ~ ., data=tmp_data)
#                      })
# loo_models_delta_r2 <- data.frame(colnames(regression_data)[-1],
#                                   sapply(position_models, 
#                                          function(x) summary(x)$r.squared),
#                                   matrix(unlist(strsplit(colnames(regression_data)[-1], 
#                                                          split="_")),
#                                          ncol=2, byrow=T))
# colnames(loo_models_delta_r2) <- c("var", "r2", "type", "pos")
# loo_models_delta_r2$delta_r2 <- summary(full_model)$r.squared - loo_models_delta_r2$r2
# loo_models_delta_r2$pos <- factor(loo_models_delta_r2$pos, levels=1:20)
# loo_plot <- ggplot(loo_models_delta_r2, aes(x=pos, y=delta_r2, fill=type)) + 
#   geom_col(position="stack") + theme_bw() + xlab("position") + ylab(expr(Delta*r^2)) + 
#   ggtitle("Rate ~ spacer sequence + viral genome structure",
#           subtitle="leave-one-out models")
```

* "Unstructured" label from Lan (2020) is most predictive of guide rate
* Also informative:
    + Base-paired-ness from Huston (2021) in position 1
    + *in vitro* icSHAPE score from Sun (2021) in positions 6-9
* Spacer sequence is most infromative in positions 2, 13, 17, 18
