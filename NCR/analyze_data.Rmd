...
optimize_glmnet_hyperparameters <- function(input_data, fold_ids, 
  ...
  # 2. calculate r^2 per split across all models
  all_models_r2 <- lapply(seq_along(all_models),
                          function(alpha_id) {
  ...
  all_models_r2 <- do.call(rbind, all_models_r2)
  all_models_r2 <- aggregate(r2 ~ alpha + lambda + split, 
                             data=all_models_r2, FUN=mean)
  all_models_r2$alpha <- factor(as.character(all_models_r2$alpha), 
                                levels=as.character(alphas))
  all_models_r2$split <- factor(all_models_r2$split, 
  ...
  # 4. calculate best r^2 per split per alpha
  ...
  all_models_best_r2$split <- factor(all_models_best_r2$split,
                                     levels=c("training", "testing"))
  best_r2_plot <- ggplot(all_models_best_r2, aes(x=as.numeric(as.character(alpha)), y=r2)) + 
    geom_point() + geom_line(alpha=0.5) + 
    theme_bw() + facet_grid(split ~ ., scales="free_y") + 
  ...
  return(list(all_models=all_models, r2=all_models_r2, MSE=all_models_MSE,
              best_alpha=best_alpha, best_lambda=best_lambda, metric=metric,
              r2_plot=r2_plot, mse_plot=mse_plot, best_r2_plot=best_r2_plot))
}
...
model1 <- list(r2=optimize_glmnet_hyperparameters(regression_data_onehot,
               ...
               MSE=optimize_glmnet_hyperparameters(regression_data_onehot,
                                                   fold_ids,
                                                   metric="mse"))
((model1$r2$best_r2_plot + ggtitle(bquote("optimize"~r^2))) + 
...
plot_coefs(model1_bestfit, model1$MSE$best_alpha, model1$MSE$best_lambda)
# plot fit across folds
plot_fold_fits(regression_data_onehot, fold_ids, 
...
model2 <- list(r2=optimize_glmnet_hyperparameters(model2_data,
               ...
               MSE=optimize_glmnet_hyperparameters(model2_data,
                                                   fold_ids,
                                                   metric="mse"))
((model2$r2$best_r2_plot + ggtitle(bquote("optimize"~r^2))) + 
...
plot_coefs(model2_bestfit, model2$MSE$best_alpha, model2$MSE$best_lambda)

# plot fit across folds
plot_fold_fits(model2_data, fold_ids, 
...
model3 <- list(r2=optimize_glmnet_hyperparameters(model3_data,
               ...
               MSE=optimize_glmnet_hyperparameters(model3_data,
                                                   fold_ids,
                                                   metric="mse"))
((model3$r2$best_r2_plot + ggtitle(bquote("optimize"~r^2))) + 
...
plot_coefs(model3_bestfit, model3$MSE$best_alpha, model3$MSE$best_lambda)
# plot fit across folds
plot_fold_fits(model3_data, fold_ids, 
