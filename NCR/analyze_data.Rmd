...
guide_features$downstream_U <- sapply(seq(nrow(guide_features)),
                                      function(x) {
                                        ...
                                        region_end <- region_start + downstream_U_window - 1
...
guide_features$antitag_pos1 <- sapply(seq(nrow(guide_features)),
                                      function(x) {
                                        tmp_position <- guide_features$start[x] + 
                                          nchar(guide_features$spacer[x])
                                        viral_genome$base[tmp_position]
                                      })
```
...
(ggplot(guide_rate, aes(x=downstream_U, y=Estimate)) + 
   geom_point() + geom_smooth(method="lm", formula=y~x) + theme_bw() + 
   xlab(paste0("% U in downstream ", downstream_U_window, "nt in viral genome")) + 
   ylab("rate above no activator (RFU/min)") +
   annotate(geom="text", x=Inf, y=75, hjust="inward", vjust="inward", col="red",
            label=bquote(rho~"="~.(round(with(guide_rate, 
  ...
  (ggplot(guide_rate, aes(x=downstream_unstructured_U, y=Estimate)) + 
     geom_point() + geom_smooth(method="lm", formula=y~x) + theme_bw() + 
     xlab(paste0("% unstructured U in downstream ", downstream_U_window, "nt in viral genome")) + 
     ylab("rate above no activator (RFU/min)") +
     annotate(geom="text", x=Inf, y=75, hjust="inward", vjust="inward", col="red",
              label=bquote(rho~"="~.(round(with(guide_rate, 
...
ggplot(guide_rate, aes(x=gRNA_MFE, y=Estimate)) + 
  geom_point() + geom_smooth(method="lm", formula=y~x) + theme_bw() + 
  xlab("gRNA MFE (kcal/mol)") + ylab("rate above no activator (RFU/min)") +
  annotate(geom="text", x=-Inf, y=75, hjust="inward", vjust="inward", col="red",
           label=bquote(rho~"="~.(round(with(guide_rate, 
                                             cor.test(gRNA_MFE, 
                                                      Estimate,
...
```{r figure_2a}
ggplot(guide_rate, aes(x=Estimate)) + 
  geom_histogram(binwidth=5) + theme_classic() + 
  xlab("rate above no activator (RFU/min)")
```
...
virus_v_gblock_data <- subset(guide_rate, NCR.id != "NCR_1352" & nchar(spacer)==20)
(ggplot(virus_v_gblock_data, aes(x=Estimate)) + 
  ...
  (ggplot(virus_v_gblock_data, aes(x=Estimate, y=gblock_rate_round2)) + 
     ...
     xlab("rate above background (RFU/min): viral genome") + 
     ylab("rate above background (RFU/min): gblock") + 
...
model1_MSE_r2 <- aggregate(r2~split, 
                           ...
                           FUN=mean)
(model1_coef_plot + theme_classic() + labs(col="", shape="") + 
   ggtitle("elastic net regression", 
           subtitle=bquote("mean testing"~r^2~"="~
                             .(round(model1_MSE_r2$r2[model1_MSE_r2$split=="testing"], 
                                                     digits=3))))) + 
  (ggplot_violin(subset(guide_rate, nchar(spacer)==20), 
                 x_var="antitag_pos1", x_lab="first position in PFS") + 
     theme_classic() + guides(fill=F) + 
     ggtitle("linear regression", 
             bquote("mean testing"~r^2~"="~.(round(model7_r2$r2[model7_r2$split=="testing"],
                                                   digits=3)))))
```
...
```{r suppl_figure_1}
(all_plots[["527"]] + theme_classic() + ylab("RFU") + 
   labs(col="") + ggtitle("", subtitle="target position: 7720-7739")) + 
  (all_plots[["505"]] + theme_classic() + ylab("RFU") + 
     labs(col="") + ggtitle("", subtitle="target position: 510-529") + ylim(2500, 7000))
```
...
(ggplot(subset(noActivator_rate, crRNA_spacer_basepairs %in% c(0, 4, 10, 16)),
        aes(x=as.numeric(as.character(crRNA_spacer_basepairs)), y=Estimate)) + 
...
```{r suppl_figure_2C}
(ggplot(noActivator_rate, aes(x=direct_repeat, y=Estimate)) + 
    theme_classic() + theme(axis.text.x=element_blank()) + 
    geom_dotplot(binaxis="y", stackdir="center", binwidth=0.25) + 
    xlab("") + ylab("background rate (RFU/min)")) / 
...
```{r suppl_figure_2B}
(ggplot(guide_rate, aes(x=GC_content, y=Estimate)) + theme_classic() + 
     geom_point() + geom_smooth(method=lm, formula=y~x) + 
     xlab("spacer %GC") + ylab("rate above no activator (RFU/min)"))
```

Suppl. Figure 3A

```{r suppl_figure_3A}
(manfredonia_huston + theme_classic()) + 
  (lan_huston + theme_classic()) + 
  (sun_invivo_huston + theme_classic()) + 
  (sun_invitro_huston + theme_classic()) +
  (manfredonia_sun_invitro + theme_classic()) + 
  (lan_sun_invitro + theme_classic()) + 
  (sun_invivo_sun_invitro + theme_classic()) + 
  plot_spacer() +
  (manfredonia_sun_invivo + theme_classic()) + 
  (lan_sun_invivo + theme_classic()) + 
  plot_spacer() + plot_spacer() +
  (manfredonia_lan + theme_classic()) + 
...
```{r suppl_figure_3B}
((ggplot(guide_rate, aes(x=lan, y=Estimate)) + theme_classic() +
    geom_point() + geom_smooth(method=lm, formula=y~x) +
    xlab("% unstructured") + ylab("rate above noa activator (RFU/min)") + 
    ggtitle("Lan (2020)", subtitle="unstructuredness")) + 
    (ggplot(guide_rate, aes(x=huston, y=Estimate)) + theme_classic() +
       geom_point() + geom_smooth(method=lm, formula=y~x) +
       xlab("% double-stranded") + ylab("rate above no activator (RFU/min)") +
       ggtitle("Huston (2021)", subtitle="double-stranded folded viral genome")) +
    (ggplot(guide_rate, aes(x=sun_invivo, y=Estimate)) + theme_classic() +
       geom_point() + geom_smooth(method=lm, formula=y~x) +
       xlab("mean icSHAPE score") + ylab("rate above no activator (RFU/min)") +
       ggtitle("Sun (2021)", subtitle="mean icSHAPE score in vivo")))
```
...
ggplot(model1_rf_regression_importance) + 
    ...
    annotate("text", x=0.5, y=max(model1_rf_regression_importance$increaseMSE) + 1, 
             label="spacer,  5' : 3'", 
             col="red", hjust="inward", vjust="inward") + 
    xlab("position") + ylab("mean decrease in accuracy") + theme_classic() +
  ggtitle("random forest", subtitle=bquote(r^2~"="~.(round(model1_rf_regression_r2, digits=3)))) + 
  labs(col="", shape="")
```
...
```{r suppl_figure_5}
(ggseqlogo(topSpacers_info, method="custom") + 
  ggtitle(paste("Top", num_spacers, "spacers")) + ylim(0, 0.25)) / 
  (ggseqlogo(bottomSpacers_info, method="custom") +
     ggtitle(paste("Bottom", num_spacers, "spacers")) + ylim(0, 0.25))
```

```{r save_image}
save.image(file=file.path(here(), "NCR", ".RData"))
```