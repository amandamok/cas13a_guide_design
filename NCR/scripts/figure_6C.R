...
plate_data <- lapply(seq(nrow(platemap)),
                     function(x) {
                       ...
                       f10_timepoint <- which(tmp_time==10)
                       tmp_RFU <- plate_data[[platemap$plate_well[x]]]
                       f10_RFU <- tmp_RFU[f10_timepoint]
                       norm_RFU <- tmp_RFU / f10_RFU
...
endpoint_pvalues$y.position <- rep(seq(from=7.3, to=4.3, 
                                       length.out=length(unique(platemap$conc))-1), 
                                   times=2)
...
figure_6C <- ggplot(endpoint_means, aes(x=conc, y=RFU)) + 
  geom_col(aes(fill=pvalue_label)) + theme_classic(base_size=8) + 
...
ggsave(filename=file.path(figure_dir, "figure_6C.pdf"),
       plot=figure_6C,
