...
variants_platemap$sample <- ""
for(x in seq_along(variant_samples)) {
  variants_platemap$sample[variants_platemap$plate_col %in% c(2*x-1, 2*x)] <- variant_samples[x]
}
variants_platemap$sample <- factor(variants_platemap$sample, levels=variant_samples)
...
variants_data <- lapply(seq(nrow(variants_platemap)),
                        function(x) {
                          ...
                          data.frame(time=variants_data$Time..s.,
                                     RFU=as.numeric(variants_data[[tmp_well]]),
                                     well=tmp_well,
                                     multiplex=variants_platemap$multiplex[x],
                                     variant=variants_platemap$sample[x])
                        })
variants_data <- do.call(rbind, variants_data)
variants_data$time <- round(variants_data$time/60)
...
figure_5D <- ggplot(variant_slopes, aes(x=variant, y=Estimate, 
...
ggsave(filename=file.path(figure_dir, "figure_5D.pdf"),
       plot=figure_5D,
